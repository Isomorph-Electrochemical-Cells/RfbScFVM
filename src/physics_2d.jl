function darcy_outflow!(f, u, node, data)
    # dom_idx = data.dom_idx[data.bnd2dom[node.region]]
    # f[data.idx[dom_idx].p] = data.boundary.v[node.region]
...
function heat_outflow!(f, u, bnode, data)
    if bnode.region == data.bnd[:el_neg_outflow] ||
       bnode.region == data.bnd[:el_pos_outflow]
        ...
        elseif bnode.region == data.bnd[:el_pos_outflow]
            v_out = PE0 * data.boundary.v_out_pos
            cpᵥ = data.electrolyte.cpᵥ
...
function σl_eff_2d(u, region, data, deff_factor, node=0)
...
function system_flux_2d!(f, u, edge, data)
    # Flux of Darcy's law
    ...
    if edge.region == data.dom[:el_neg] || edge.region == data.dom[:el_pos]
        ...
        σl_eff_avg = (σl_eff_2d(u, edge.region, data, deff_factor, 1) +
                      σl_eff_2d(u, edge.region, data, deff_factor, 2))/2
...
function system_bcondition_2d!(f, u, bnode, data)
    ...
    @inline species_outflow!(f, u, bnode, data)
...
#       ∂ₜs(u) + ∇⋅j + r(u) - f = 0
function system_reaction_2d!(f, u, node, data)
    f .= 0
    ...
    if region == data.dom[:el_neg]
        ...
        η_l = Δϕ_l - ϕ_eq(r, u, idx_c2u, temp)
    ...
    elseif region == data.dom[:el_pos]
        ...
        idx_c2u = data.idx[region].c
        η_r = Δϕ_r - ϕ_eq(r, u, idx_c2u, temp)
...
function system_storage_2d!(f, u, node, data)
    ϵL0 = data.scaling_params.ϵL0
    PE0 = data.scaling_params.PE0
...
function system_edgereaction_2d!(f, u, edge, data)
    ...
    if data.study.non_isothermal
        ...
        elseif region == data.dom[:domain_id_el_neg]
            σₗ = (σl_eff_2d(u, region, data, deff_factor, 1) +
                 σl_eff_2d(u, region, data, deff_factor, 2)) / 2
...
function system_breaction_2d!(f, u, bnode, data)
    ...
    @inline system_bcondition_2d!(f, u, bnode, data)
    #end
    #alloc > 0 && @show alloc
...
function physics_2d(prms)
    num_eqs = num_variables_2d(prms.idx)
    VoronoiFVM.Physics(num_species  = num_eqs,
                       storage      = system_storage_2d!,
                       bstorage     = system_bstorage!,
                       flux         = system_flux_2d!,
                       bflux        = system_bflux_2d!,
                       reaction     = system_reaction_2d!,
